...
const mockTrendData = (category: string) => {
  ...
  const getMetrics = (cat: string) => {
    switch (cat) {
      case 'lipid_panel': return ['total_cholesterol', 'ldl_cholesterol', 'hdl_cholesterol', 'triglycerides', 'non_hdl_cholesterol'];
      case 'glucose_metabolism': return ['fasting_glucose', 'hba1c', 'insulin', 'estimated_average_glucose'];
      case 'renal_function': return ['creatinine', 'estimated_gfr', 'urea', 'uric_acid', 'albumin'];
      default: return [];
    }
  };
...
export const handlers = [
  ...
  http.get(`${API_URL}/reports/:id/observations`, async () => {
    ...
    const observations = Array.from({ length: 15 }).map((_, i) => ({
      id: `obs-${i + 1}`,
      name: ['Total Cholesterol', 'HDL', 'LDL', 'Triglycerides', 'Glucose', 'Creatinine'][i % 6] || `Test ${i}`,
