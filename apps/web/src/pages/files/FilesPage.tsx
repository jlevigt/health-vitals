import { AlertCircle, CheckCircle2, Clock, File, FileText, Upload } from "lucide-react";
        ...
        <div className="overflow-y-auto flex-1">
          {loading ? (
            ...
            <div className="p-12 text-center text-error flex flex-col items-center gap-2">
              ...
              <p>{error}</p>
              <button onClick={fetchFiles} className="text-sm underline hover:text-red-700">
                Retry
              </button>
            </div>
          ) : files.length === 0 ? (
            ...
            <div className="divide-y divide-outline-variant">
              {files.map((file) => (
                <div
                  key={file.id}
                  className="grid grid-cols-[1fr_auto_auto] gap-4 px-6 py-4 items-center hover:bg-surface-variant/5 transition-colors"
                >
                  ...
                  <div className="flex justify-center">{getStatusBadge(file.status)}</div>
                </div>
              ))}
            </div>
          )}
