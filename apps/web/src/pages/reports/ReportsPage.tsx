      ...
      <div className="shrink-0 flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-on-surface-variant">
            View and manage your processed clinical documents.
          </p>
        </div>
      </div>
...
const ReportRow: React.FC<{
  ...
  useEffect(() => {
    if (isExpanded && !fetched) {
      setObsLoading(true);
      api
        .get<Observation[]>(`/reports/${report.id}/observations`)
        .then((res) => {
  ...
  const currentObs = observations.slice((obsPage - 1) * OBS_PER_PAGE, obsPage * OBS_PER_PAGE);
    ...
    <div className="border-b border-outline-variant last:border-0">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-on-surface truncate max-w-[200px] sm:max-w-md">
              {report.fileName}
            </p>
        ...
        <div className="text-xs text-on-surface-variant font-medium">{report.date}</div>
            ...
            <div className="bg-white rounded-xl border border-outline-variant overflow-hidden">
              ...
              {obsLoading ? (
                ...
                <div className="p-4 text-center text-xs text-on-surface-variant">
                  No observations found for this report.
                </div>
              )}
            </div>
          </div>
